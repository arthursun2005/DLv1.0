...
DL.addBlock(b);
var b  = new Block(0,-10,-120,w1,w1,120);
DL.addBlock(b);
// b,length,diameter,thickness,changeInY,changeInD,turn,f
for (var i = 0; i < intervals1.length; i++) {
  DL.addBlock(createNormalBlockFromInterval(DL.blocks[DL.blocks.length-1],120*intervals1[i],w1,w1,0,0));
}
function autoCamera(world){
  ...
  camera.position.y = line.p.y+l1*2;
}
var dtime;
function animate() {
